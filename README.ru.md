...
не отыгрывается — список в разделе
[«Что не смоделировано»](#что-не-смоделировано). Если такая карта стоит на любом
из столов, оверлей пишет это прямо на карточке шансов
(`точность ~85% · не смоделировано: <карты>`), а не выдаёт уверенное число,
за которое не может отвечать.</sub>

[English README](README.md) · [Инструкция по установке](docs/INSTALL.ru.md) · macOS 12+ · Python 3.11+ · MIT
...
Скрипт создаёт venv на лучшем найденном Python — 3.13 и arm64, если они есть, —
и ставит туда PyObjC. Затем проверь окружение:
